...
def find_two_most_similar_users(query_user, candidates, user_ids, user_movies_dict):
    ...
    most_similar = max(user_candidates, key=similarity) # get the user with the highest jaccard similarity
    user_candidates.remove(most_similar) # remove the user with the highest jaccard similarity from the candidates
    second_most_similar = max(user_candidates, key=similarity) # get the second user with the highest jaccard similarity
    two_most_similar_users = [(most_similar, jaccard_similarity(set(user_movies_dict[query_user]), set(user_movies_dict[most_similar]))), (second_most_similar, jaccard_similarity(set(user_movies_dict[query_user]), set(user_movies_dict[second_most_similar])))] # create list of the two most similar users
    two_most_similar_df = pd.DataFrame(two_most_similar_users, columns = ['User', 'Similarity']) # create dataframe of the two most similar users
...
def recommend_movies(query_user, candidates, user_ids, user_movies_dict, rating, movie_df):
    ...
    if movies_intersection: # If the intersection is not empty, return a DataFrame with the recommended movies
        ...
        movies_intersection_df.sort_values(by='rating', ascending=False, inplace=True)
        ...
        else: # If the two most similar users have less than five movies in common
            # Fill the remaining recommendations with top rated movies from the most similar user
            top_rated = get_rated_movies(userId1, rating)
            # Sort the movies by rating
            top_rated.sort(key=lambda x: x[1], reverse=True)
            # Return five movie recommendations, the ones movie_intersection_df plus movies from the most similar user until we get to five movies
            num_movies_from_most_similar = 5 - len(movies_intersection_df)
            # Restrict number of top rated movies from most similar user to num_movies_from_most_similar
            top_rated = top_rated[:num_movies_from_most_similar]
            # Replace movie Id with the movie name using the movie_df DataFrame
            top_rated = [(movie_df[movie_df['movieId']==movie[0]]['title'].values[0], movie[1]) for movie in top_rated]
            # Concatenate movies in the intersection with movies rated by the most similar user
            movies_intersection_df = pd.concat([movies_intersection_df, pd.DataFrame(top_rated[:num_movies_from_most_similar], columns = ['movie', 'rating'])])
    ...
    else: # If the intersection is empty, return the top-rated movies of the most similar user
        # Get the top-rated movies of the most similar user
        top_rated = get_rated_movies(userId1, rating)
        # Sort the movies by rating
        top_rated.sort(key=lambda x: x[1], reverse=True)
        # Return the top rated movies by this user
        return top_rated[:5]